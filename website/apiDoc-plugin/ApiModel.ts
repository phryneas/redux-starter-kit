...
import {
  ApiItem,
  ApiItemKind
} from '@microsoft/api-extractor-model/lib/items/ApiItem'
import { ApiItemContainerMixin } from '@microsoft/api-extractor-model/lib/mixins/ApiItemContainerMixin'
import { ApiPackage } from '@microsoft/api-extractor-model/lib/model/ApiPackage'
import { PackageName } from '@rushstack/node-core-library'
import {
  ModelReferenceResolver,
  IResolveDeclarationReferenceResult
} from '@microsoft/api-extractor-model/lib/model/ModelReferenceResolver'
...
export class ApiModel extends ApiItemContainerMixin(ApiItem) {
  ...
  public addMember(member: ApiPackage): void {
    if (member.kind !== ApiItemKind.Package) {
      throw new Error(
        'Only items of type ApiPackage may be added to an ApiModel'
      )
    }
  ...
  public tryGetPackageByName(packageName: string): ApiPackage | undefined {
    ...
    if (this._packagesByName === undefined) {
      ...
      const unscopedMap: Map<string, ApiPackage | undefined> = new Map<
        string,
      ...
      for (const apiPackage of this.packages) {
        if (this._packagesByName.get(apiPackage.name)) {
          ...
          throw new Error(
            `The model contains multiple packages with the name ${apiPackage.name}`
          )
        }

        this._packagesByName.set(apiPackage.name, apiPackage)

        const unscopedName: string = PackageName.parse(apiPackage.name)
          .unscopedName
